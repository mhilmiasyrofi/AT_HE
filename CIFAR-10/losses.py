...
def alp_loss(model, x_natural, x_adv, y, y_adv, optimizer, step_size=0.003, epsilon=8./255., perturb_steps=10,
    ...
    if loss=='alp':
        ...
        loss_robust = 0.5 * criterion_loss(logits, y, cm = m) + 0.5 * criterion_loss(logits_adv, y_adv, cm = m)
        loss_alp = F.mse_loss(logits, logits_adv)
        loss = loss_robust + beta * loss_alp
    else:
        raise RuntimeError('A error occurred')
